 ...
 * - Adapted to AVTextFormatContext API
 * - Custom AVIOContext implemented to redirect writer output to av_log
 *
 * 07.2023
 * --------------------------------------------------------
...
// FFmpegKit: global variables marked __thread
...
static __thread struct AVTextFormatSection sections[] = {
...
static __thread AVMutex log_mutex = AV_MUTEX_INITIALIZER;
...
static void print_section(SectionID id, int level)
{
    ...
    av_log(NULL, AV_LOG_INFO, "%*c  %s", level * 4, ' ', section->name);
    if (section->unique_name)
        av_log(NULL, AV_LOG_INFO, "/%s", section->unique_name);
    av_log(NULL, AV_LOG_INFO, "\n");
...
{
    av_log(NULL, AV_LOG_INFO, "Sections:\n"
...
void ffprobe_var_cleanup() {
    ...
    // Reset static AVMutex log_mutex? No, it's mutex. But in kit it is __thread and re-inited.
    // If it is __thread, we need to destroy old and re-init?
...
static void ffprobe_cleanup(int ret)
{
    ...
    av_freep(&show_data_hash); // Free this as well
...
int ffprobe_execute(int argc, char **argv)
{
    ...
    // Set global program_name for logging
    ...
    if (savedCode == 0) {
        ...
        // Defined inside function to allow address of __thread variables
        ...
        if (output_filename) {
            ...
            // FFmpegKit: Redirect stdout writes to av_log via custom AVIO
            uint8_t *avio_buffer = av_malloc(4096);
            if (!avio_buffer) {
                ret = AVERROR(ENOMEM);
                goto end;
            }
            AVIOContext *log_pb = avio_alloc_context(avio_buffer, 4096, 1, NULL, NULL, avio_log_write, NULL);
            if (!log_pb) {
                av_free(avio_buffer);
                ret = AVERROR(ENOMEM);
                goto end;
