...
#define PRINT_CODEC_SUPPORTED(codec, config, type, name, elem, fmt, ...)        \
    do {                                                                        \
        ...
        if (avcodec_get_supported_config(NULL, codec, config, 0,                \
                                         (const void **) &elem, &num) >= 0) {   \
...
static void show_help_bsf(const char *name)
{
    ...
    if (bsf->codec_ids) {
        const enum AVCodecID *c;
        av_log(NULL, AV_LOG_ERROR, "    Supported codecs:");
        for (c = bsf->codec_ids; *c != AV_CODEC_ID_NONE; c++) {
            const AVCodecDescriptor *desc = avcodec_descriptor_get(*c);
            if (desc)
                av_log(NULL, AV_LOG_ERROR, " %s", desc->name);
        }
        av_log(NULL, AV_LOG_ERROR, "\n");
    }
    ...
    while ((filter = av_filter_iterate(&opaque))) {
        ...
        *descr_cur = 0;
        av_log(NULL, AV_LOG_ERROR, " %c%c%c %-17s %-10s %s\n",
               ...
               filter->flags & AVFILTER_FLAG_SUPPORT_TIMELINE ? 'C' : '.',
