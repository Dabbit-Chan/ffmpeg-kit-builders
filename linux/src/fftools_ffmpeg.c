...
__thread atomic_uint nb_output_dumped = ATOMIC_VAR_INIT(0);
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts)
{
    ...
    static __thread int first_report = 1;
    ...
    for (InputStream *ist = ist_iter(NULL); ist; ist = ist_iter(ist)) {
        for (int j = 0; j < ist->nb_filters; j++) {
            if (!filtergraph_is_simple(ist->filters[j]->graph)) {
                // CHANGED: ist->file_index to ist->file->index
                av_log(NULL, AV_LOG_INFO, "  Stream #%d:%d (%s) -> %s",
                       ist->file->index, ist->st->index, ist->dec ? ist->dec->name : "?",
                       ist->filters[j]->name);
...
{
    int i, ret, key;
    static __thread int64_t last_time;
...
static int transcode(Scheduler *sch)
{
    ...
    while (!received_sigterm && !cancelRequested(globalSessionId)) {
        ...
        if (sch_wait(sch, stats_period, &transcode_ts))
            break;
...
void ffmpeg_var_cleanup(void)
{
    ...
    longjmp_value = 0;
    want_sdp = 1;
    // enc_stats_files handled in mux_init cleanup or needs thread local reset
...
int ffmpeg_execute(int argc, char **argv)
{
    ...
    if (savedCode == 0) {
        ...
        // parse_loglevel now in cmdutils / options parsing
