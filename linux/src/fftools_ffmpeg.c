                                             ...
                                             AVFrame *frame)
{
  ...
  const int64_t start_time = (of->start_time == AV_NOPTS_VALUE) ? 0 : of->start_time;
                                   ...
                                   int dup_idx)
{
  ...
  else if (kf->pexpr)
  {
    ...
    if (res)
    {
      ...
      goto force_keyframe;
           ...
           (in_picture->flags & AV_FRAME_FLAG_KEY) == 1 && !dup_idx)
  {
    goto force_keyframe;
  }
  else if (kf->type == KF_FORCE_SOURCE_NO_DROP && !dup_idx)
  {
    ...
    if (((in_picture->flags & AV_FRAME_FLAG_KEY) == 1) || kf->dropped_keyframe)
      goto force_keyframe;
  }
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
  ...
  {
    if (pkt->pts == AV_NOPTS_VALUE ? ist->pts < ost->ts_copy_start : pkt->pts < av_rescale_q(ost->ts_copy_start, AV_TIME_BASE_Q, ist->st->time_base))
      return;
  }
...
static int process_subtitle(InputStream *ist, AVSubtitle *subtitle, int *got_output)
{
  ...
  for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost))
  {
    if (!check_output_constraints(ist, ost) || !ost->enc_ctx || ost->enc_ctx->codec_type != AVMEDIA_TYPE_SUBTITLE)
      continue;
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt, int no_eof)
{
  ...
  while (ist->decoding_needed)
  {
    ...
    case AVMEDIA_TYPE_VIDEO:
      ...
      if (!repeating || !pkt || got_output)
      {
        ...
        {
          int ticks = ist->last_pkt_repeat_pict >= 0 ? ist->last_pkt_repeat_pict + 1 : 1;
          duration_dts = ((int64_t)AV_TIME_BASE * ist->dec_ctx->framerate.den * ticks) / ist->dec_ctx->framerate.num / 1;
          /*
  ...
  if (!ist->decoding_needed && pkt)
  {
    ...
    case AVMEDIA_TYPE_VIDEO:
      ...
      {
        int ticks = ist->last_pkt_repeat_pict >= 0 ? ist->last_pkt_repeat_pict + 1 : 1;
        ist->next_dts = ((int64_t)AV_TIME_BASE * ist->dec_ctx->framerate.den * ticks) / ist->dec_ctx->framerate.num / 1;
        /*
        int ticks = ist->last_pkt_repeat_pict >= 0 ?
                      ist->last_pkt_repeat_pict + 1  :
                      ist->dec_ctx->ticks_per_frame;
...
static int init_output_stream_streamcopy(OutputStream *ost)
{
  ...
  {
    ost->ts_copy_start = (of->start_time == AV_NOPTS_VALUE) ? 0 : of->start_time;
...
#define OFFSET(x) offsetof(OptionsContext, x)
  OptionDef options[] = {
      ...
      {"map_channel", HAS_ARG | OPT_EXPERT | OPT_PERFILE | OPT_OUTPUT, {.func_arg = opt_map_channel}, "map an audio channel from one stream to another (deprecated)", "file.stream.channel[:syncfile.syncstream]"},
#endif
      ...
      {"shortest_buf_duration", HAS_ARG | OPT_FLOAT | OPT_EXPERT | OPT_OFFSET | OPT_OUTPUT, {.off = OFFSET(shortest_buf_duration)}, "maximum buffering duration (in seconds) for the -shortest option"},
      ...
      {"thread_queue_size", HAS_ARG | OPT_INT | OPT_OFFSET | OPT_EXPERT | OPT_INPUT | OPT_OUTPUT, {.off = OFFSET(thread_queue_size)}, "set the maximum number of queued packets from the demuxer"},
      {"find_stream_info", OPT_BOOL | OPT_INPUT | OPT_EXPERT | OPT_OFFSET, {.off = OFFSET(find_stream_info)}, "read and decode the streams to fill missing information with heuristics"},
      ...
      {"stats_enc_pre_fmt", HAS_ARG | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT | OPT_STRING, {.off = OFFSET(enc_stats_pre_fmt)}, "format of the stats written with -stats_enc_pre"},
      {"stats_enc_post_fmt", HAS_ARG | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT | OPT_STRING, {.off = OFFSET(enc_stats_post_fmt)}, "format of the stats written with -stats_enc_post"},
      {"stats_mux_pre_fmt", HAS_ARG | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT | OPT_STRING, {.off = OFFSET(mux_stats_fmt)}, "format of the stats written with -stats_mux_pre"},
      ...
      {"display_rotation", OPT_VIDEO | HAS_ARG | OPT_DOUBLE | OPT_SPEC | OPT_INPUT, {.off = OFFSET(display_rotations)}, "set pure counter-clockwise rotation in degrees for stream(s)", "angle"},
      {"display_hflip", OPT_VIDEO | OPT_BOOL | OPT_SPEC | OPT_INPUT, {.off = OFFSET(display_hflips)}, "set display horizontal flip for stream(s) "
                                                                                                      "(overrides any display rotation if it is not set)"},
      {"display_vflip", OPT_VIDEO | OPT_BOOL | OPT_SPEC | OPT_INPUT, {.off = OFFSET(display_vflips)}, "set display vertical flip for stream(s) "
                                                                                                      "(overrides any display rotation if it is not set)"},
      ...
      {"fix_sub_duration_heartbeat", OPT_VIDEO | OPT_BOOL | OPT_EXPERT | OPT_SPEC | OPT_OUTPUT, {.off = OFFSET(fix_sub_duration_heartbeat)}, "set this video output stream to be a heartbeat stream for "
