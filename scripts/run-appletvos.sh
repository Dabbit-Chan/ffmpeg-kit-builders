...
build_libass() {
  ...
  if istvossimulator; then
    config+=" --host=$meson_cpu_family-apple-tvos-simulator"
  else
...
-Dcoretext=enabled \
-Dasm=enabled"
  sed -i'.bak' -e "s/== 'darwin'/ in ['darwin', 'tvos', 'tvos-simulator']/g" "$src_dir/$lib/meson.build"
  generic_meson "$meson_options"
  disable_nonessential "$src_dir/$lib"
  do_ninja_and_ninja_install
  change_dir "$src_dir"
...
build_libdav1d() {
  ...
  change_dir "$src_dir/$lib"
  if istvossimulator; then
  sed -i'.bak' -e "s/'tvos'/'tvos', 'tvos-simulator'/g" "$src_dir/$lib/meson.build"
  fi
...
build_libfribidi() {
  ...
  if istvossimulator; then
  export CFLAGS="-arch $host_arch -isysroot $SDKROOT -mtvos-simulator-version-min=12.0"
  else
  export CFLAGS="-arch $host_arch -isysroot $SDKROOT -mtvos-version-min=12.0"
  fi
  ...
  if istvossimulator; then
    do_autogen "--host=$meson_cpu_family-apple-tvos-simulator --disable-shared --enable-static --disable-docs --disable-deprecated --prefix=$dependency_install_prefix"
  else
    do_autogen "--host=$meson_cpu_family-apple-tvos --disable-shared --enable-static --disable-docs --disable-deprecated --prefix=$dependency_install_prefix"
  fi
  # {
  #   ./configure --host=aarch64-apple-tvos --disable-shared --enable-static --disable-docs --disable-deprecated --prefix=$dependency_install_prefix
...
-Dc_link_args=\"-arch $host_arch -L${dependency_install_prefix}/lib -liconv -lintl -lresolv -framework CoreFoundation -isysroot ${TVOS_SYSROOT}\" \
-Dcpp_link_args=\"-arch $host_arch -L${dependency_install_prefix}/lib -liconv -lintl -lresolv -framework CoreFoundation -isysroot ${TVOS_SYSROOT}\" \
...
all_platforms="${all_platforms} arm64-appletvsimulator-gcc"' "$src_dir/$lib/configure"
    sed -i '' 's/-mtvos-version-min=\${TVOS_VERSION_MIN}"$/-mtvos-simulator-version-min=${TVOS_VERSION_MIN}"/g' "$src_dir/$lib/build/make/configure.sh"
...
-DCMAKE_C_FLAGS=\"-Wno-unknown-warning-option -Wno-error -Wno-typedef-redefinition -Wno-header-guard -I$src_dir/$lib/src_base -I$src_dir/$lib/src_base/neon\""
  if [[ "$host_arch" == "arm64" ]]; then
    ...
    # Remove the -static linker flag that breaks tvOS builds
