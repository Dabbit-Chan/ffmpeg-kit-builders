...
build_ladspa() {
  echo "INFO: ladspa is not supported on the OpenHarmony minimum bundle yet." >>"$LOG_FILE"
  disable_library "ladspa"
}
...
#------------------------------------------------------------------------------
# build_ohcodec           # config_options+= --enable-ohcodec             # enable OpenHarmony Codec support [no]
build_ohcodec() {
  # ohcodec is provided by the OpenHarmony Native SDK as a set of native
  # media headers and shared libraries under
  # $OHOS_SDK_HOME/sysroot/usr/include/multimedia/player_framework/
  # and $OHOS_SDK_HOME/sysroot/usr/lib/<triple>/libnative_media_*.so.
  # FFmpeg links against those at FFmpeg configure time, so we only need
  # to verify the SDK actually has them.
  local required_headers=(
    "$OHOS_SDK_HOME/sysroot/usr/include/multimedia/player_framework/native_avcodec_base.h"
    "$OHOS_SDK_HOME/sysroot/usr/include/multimedia/player_framework/native_avcodec_videodecoder.h"
    "$OHOS_SDK_HOME/sysroot/usr/include/multimedia/player_framework/native_avcodec_videoencoder.h"
  )
  for header in "${required_headers[@]}"; do
    if [[ ! -f "$header" ]]; then
      echo "WARNING: ohcodec header missing: $header. Disabling ohcodec." | tee -a "$LOG_FILE"
      disable_library "ohcodec"
      return 0
    fi
  done
  echo "INFO: ohcodec is provided by the OpenHarmony Native SDK. No external library to build." >>"$LOG_FILE"
}
#endregion---------------------------------------------------------------------    
...
get_meson_cross_file() {
  local variant_name="$1"      # e.g., "librist"
    ...
    # 1. Generate the BASE file if it doesn't exist (Harmony Logic)
...
get_local_meson_cross_with_propeties() {
  local local_dir="$1"
	...
	copy_path "$src_dir/$host_name-meson-cross.harmony.txt" "$local_dir/meson-cross.harmony.txt"
}
