...
build_libusb() {
  ...
  change_dir "$src_dir/$lib"
  generic_configure_make_install "--disable-udev --enable-static --disable-shared"
  change_dir "$src_dir"
}
build_sdl12_compat() {
  build_sdl2 1
...
build_libdrm() {
  if [[ $disable_libdrm != 1 && $enable_libdrm == 1 ]] || [[ -n "$1" ]]; then
  activate_meson
  ...
  change_dir "$src_dir"
	do_git_checkout "$repo" "$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  export LDFLAGS="-L${dependency_install_prefix}/lib"
  local meson_options=""
...
build_libv4l2() {
  if [[ ($disable_libv4l2 != 1 && $enable_libv4l2 == 1) ]] || [[ -n "$1" ]]; then
  build_libjpeg_turbo
  activate_meson
  local lib="libv4l2"
  ...
  change_dir "$src_dir"
	do_git_checkout "$repo" "$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  local meson_options=" -Ddoxygen-doc=disabled -Ddoxygen-html=false -Ddoxygen-man=false"
	generic_meson "$meson_options"
	do_ninja_and_ninja_install
  fi
}
...
build_vaapi() {
  ...
  # local meson_options=" -Dc_link_args=\"-lXau -lXdmcp -L${dependency_install_prefix}/lib\" -Dcpp_link_args=\"-lXau -lXdmcp -L${dependency_install_prefix}/lib\""
	# generic_meson "$meson_options"
	# do_ninja_and_ninja_install
  generic_configure_make_install "--enable-static --disable-shared"
  reset_cflags
...
build_xlib() {
  if [[ $disable_xlib != 1 && $enable_xlib == 1 ]] || [[ -n "$1" ]]; then
  build_xorgproto
  build_xtrans
  build_libxcb 1
  build_libxext
...
-DVULKAN_HEADERS_ENABLE_INSTALL=YES $extra_args"
  change_dir "$src_dir"
  fi
}
...
build_lzma() {
  if [[ $disable_lzma != 1 && $enable_lzma == 1 ]] || [[ -n "$1" ]]; then
...
build_chromaprint() {
  ...
  do_git_checkout "$repo" "$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DBUILD_TOOLS=OFF -DBUILD_TESTS=OFF -DFFT_LIB=fftw3"
  change_dir "$src_dir"
  fi
}
...
build_libaribcaption() {
  ...
  do_git_checkout "$repo" "$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DBUILD_TESTS=OFF -DBUILD_EXAMPLES=OFF"
  change_dir "$src_dir"
  fi
}
...
build_libcaca() {
  ...
  do_git_checkout "$repo" "$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  export LDFLAGS="$LDFLAGS -lbrotlidec -lbrotlicommon -liconv -lxml2"
...
build_libgme() {
  ...
  download_and_unpack_file "$repo" "$lib"
  change_dir "$src_dir/$lib"
	do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DENABLE_UBSAN=0"
	change_dir "$src_dir"
  fi
}
build_libsndfile() {
...
build_graphite() {
  ...
  sed -i "s/add_subdirectory(doc)/#add_subdirectory(doc)/g" CMakeLists.txt
	do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF"
	change_dir "$src_dir"
}
...
build_libmp3lame() {
  if [[ $disable_libmp3lame != 1 && $enable_libmp3lame == 1 ]] || [[ -n "$1" ]]; then
  build_mpg123
...
-DHAVE_DSHOW=0"
  do_cmake_from_build_dir "$src_dir/$lib" "$cmake_params"
  do_make_and_make_install
  ln -sf "${dependency_install_prefix}/lib/pkgconfig/opencv4.pc" "${dependency_install_prefix}/lib/pkgconfig/opencv.pc"
  reset_ldflags
  export PKG_CONFIG_PATH=$original_pkg_path
  change_dir "$src_dir"
  fi
...
build_libopenjpeg() {
  ...
  do_git_checkout "$repo" "$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DOPJ_BIG_ENDIAN=0 -DBUILD_CODEC=0"
  change_dir "$src_dir"
  fi
}
build_libogg() {
...
build_libopus() {
  if [[ $disable_libopus != 1 && $enable_libopus == 1 ]] || [[ -n "$1" ]]; then
  local lib="libopus"
...
build_vulkan_loader() {
  ...
  local repo_ver="v1.4.326"
	do_git_checkout "$repo" "$lib" "$repo_ver"
	do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release -DVULKAN_SHIM_IMPERSONATE=ON"
	change_dir "$src_dir"
}
...
build_libpulse() {
  ...
  export CFLAGS="$CFLAGS -I${dependency_install_prefix}/include"
  export LDFLAGS="$LDFLAGS -L${dependency_install_prefix}/lib -L${dependency_install_prefix}/lib/${host_target}"
  export LIBS="-lmpg123 -lmp3lame -lid3tag -lvorbisenc -lvorbis -logg -lFLAC -lopus -liconv"
...
build_libdatrie() {
  ...
  change_dir "$src_dir"
  generic_download_and_make_and_install "$repo" "$lib" "--enable-static --disable-shared --disable-doxygen-doc"
	change_dir "$src_dir"
}
build_libthai() {
  build_libdatrie
...
build_libsnappy() {
  ...
  do_git_checkout "$repo" "$lib" "$repo_ver"
	change_dir "$src_dir/$lib"
	do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DBUILD_BINARY=OFF -DSNAPPY_BUILD_TESTS=OFF -DSNAPPY_BUILD_BENCHMARKS=OFF" # extra params from deadsix27 and from new cMakeLists.txt content
	change_dir "$src_dir"
  fi
}
...
build_libsoxr() {
  ...
  do_git_checkout "$repo" "$lib" "$repo_ver"
	change_dir "$src_dir/$lib"
	do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DWITH_OPENMP=0 -DBUILD_TESTS=0 -DBUILD_EXAMPLES=0"
	change_dir "$src_dir"
  fi
}
...
build_libspeexdsp() {
  if [[ $disable_libspeex != 1 && $enable_libspeex == 1 ]] || [[ -n "$1" ]]; then
  build_libspeex
...
build_cpuinfo() {
  ...
  local repo="https://github.com/pytorch/cpuinfo"
	...
	do_git_checkout "$repo" "$lib"
	change_dir "$src_dir/$lib"
	do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF" # builds included cpuinfo bugged
	change_dir "$src_dir"
}
...
build_libtiff() {
  ...
  download_and_unpack_file "$repo" "$lib"
  change_dir "$src_dir/$lib"
  generic_configure_make_install "--enable-static --disable-shared --disable-docs --disable-tools --disable-tests"
...
build_libidn2() {
  ...
  local repo_ver="2.3.8"
	...
	download_and_unpack_file "$repo" "$lib"
	change_dir "$src_dir/$lib"
	generic_configure_make_install "--enable-static --disable-shared --with-pic --with-libunistring-prefix=$dependency_install_prefix"
	change_dir "$src_dir"
}
build_libunistring() {
  ...
  local repo_ver="1.4.1"
	...
	download_and_unpack_file "$repo" "$lib"
	change_dir "$src_dir/$lib"
	generic_configure_make_install "--enable-static --disable-shared --with-pic"
	change_dir "$src_dir"
}
build_curl() {
  build_libidn2
...
--without-gssapi \
--without-libidn2
LIBS=\"-lpsl -lidn2 -lunistring -liconv\""
	do_make_and_make_install
	reset_cppflags
	change_dir "$src_dir"
}
...
build_libvidstab() {
  ...
  change_dir "$src_dir"
	do_git_checkout "$repo" "$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  do_cmake_and_install "-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DUSE_OMP=0 -DBUILD_SHARED_LIBS=0" # '-DUSE_OMP' is on by default, but somehow libgomp ('cygwin_local_install/lib/gcc/i686-pc-cygwin/5.4.0/include/omp.h') can't be found, so '-DUSE_OMP=0' to prevent a compilation error.
	change_dir "$src_dir"
  fi
}
...
build_libx265() {
  ...
  do_cmake_from_build_dir "$src_dir/$lib/source" "-DHIGH_BIT_DEPTH=ON -DEXPORT_C_API=OFF -DENABLE_SHARED=OFF -DENABLE_CLI=OFF -DMAIN12=ON"
  do_make
  change_dir "$src_dir/$lib/10bit" 1
  do_cmake_from_build_dir "$src_dir/$lib/source" "-DHIGH_BIT_DEPTH=ON -DEXPORT_C_API=OFF -DENABLE_SHARED=OFF -DENABLE_CLI=OFF"
  do_make
...
build_libxvid() {
  ...
  # manually install static library only
  cp -fv "$src_dir/$lib/build/generic/=build/libxvidcore.a" "$dependency_install_prefix/lib/" >>"$LOG_FILE"
  cp -fv "$src_dir/$lib/src/xvid.h" "$dependency_install_prefix/include/" >>"$LOG_FILE"
...
build_vapoursynth() {
  ...
  do_git_checkout "$repo" "$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  mapfile -t missing_packages < <(get_missing_packages python3-dev)
  if [[ "${#missing_packages[*]}" -gt 0 ]]; then
    echo -e "Installing needed dependencies ${missing_packages[*]}:\n  running: \"apt-get install ${missing_packages[*]} -y\"" >>"$LOG_FILE"
    eval "apt-get install ${missing_packages[*]} -y" > >(redirect_output) 2>&1
  fi
...
build_cuda_nvcc() {
  ...
  echo "WARNING: This is a non-gpl library. Binaries including this library are non-redistributable!" >>"$LOG_FILE"
    if [[ "$bits_target" != "32" ]]; then
      ...
      change_dir "$src_dir/$lib"
      echo > "$manifest" && chmod -R u+rwx "$manifest"
      [[ -d "$src_dir/$lib/bin" ]] && { ( set -o pipefail; cp -rfv "$src_dir/$lib/bin"* "$dependency_install_prefix/" 2>&1 | sed -n "s/.*' -> '\(.*\)'/\1/p" ) >> "$manifest" || exit_message 1 "could not install $lib bin"; }
      [[ -d "$src_dir/$lib/lib" ]] && { ( set -o pipefail; cp -rfv "$src_dir/$lib/lib"* "$dependency_install_prefix/" 2>&1 | sed -n "s/.*' -> '\(.*\)'/\1/p" ) >> "$manifest" || exit_message 1 "could not install $lib lib"; }
      [[ -d "$src_dir/$lib/include" ]] && { ( set -o pipefail; cp -rfv "$src_dir/$lib/include"* "$dependency_install_prefix/" 2>&1 | sed -n "s/.*' -> '\(.*\)'/\1/p" ) >> "$manifest" || exit_message 1 "could not install $lib include"; }
      [[ -d "$src_dir/$lib/share" ]] && { ( set -o pipefail; cp -rfv "$src_dir/$lib/share"* "$dependency_install_prefix/" 2>&1 | sed -n "s/.*' -> '\(.*\)'/\1/p" ) >> "$manifest" || exit_message 1 "could not install $lib share"; }
...
build_libnpp() {
  ...
  # https://developer.download.nvidia.com/compute/cuda/redist/
    if [[ "$bits_target" != "32" ]]; then
      ...
      [[ -d "$src_dir/$lib/include" ]] && { ( set -o pipefail; cp -rfv "$src_dir/$lib/include"* "$dependency_install_prefix/" 2>&1 | sed -n "s/.*' -> '\(.*\)'/\1/p" ) >> "$manifest" || exit_message 1 "could not install $lib include"; }
      [[ -d "$src_dir/$lib/share" ]] && { ( set -o pipefail; cp -rfv "$src_dir/$lib/share"* "$dependency_install_prefix/" 2>&1 | sed -n "s/.*' -> '\(.*\)'/\1/p" ) >> "$manifest" || exit_message 1 "could not install $lib share"; }
      [[ -d "$src_dir/$lib/pkg-config" ]] && { ( set -o pipefail; cp -rfv "$src_dir/$lib/pkg-config/"* "$dependency_install_prefix/lib/pkgconfig/" 2>&1 | sed -n "s/.*' -> '\(.*\)'/\1/p" ) >> "$manifest" || exit_message 1 "could not install $lib pkg-config"; }
    else
