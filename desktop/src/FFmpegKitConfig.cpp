...
/**
 * Helper to reconstruct command string from argument list for the new _lib APIs.
 */
static std::string buildCommandString(const char* toolName, const std::shared_ptr<std::list<std::string>>& arguments) {
...
int executeFFprobe(const long sessionId, const std::shared_ptr<std::list<std::string>> arguments) {
    ...
    if (ctx) {
        ...
        if (output) {
            ...
            // Use LevelAVLogStdErr as that is what MediaInformationSession listens for
