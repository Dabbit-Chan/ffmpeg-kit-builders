...
static OptionDef real_options_template[] = {
    ...
    { "data_dump_format",      OPT_TYPE_STRING,      0, { NULL }, "set data dump format (available formats are: xxd, base64)" },
...
void ffprobe_tls_init_options(void) {
    ...
    for (int i = 0; real_options[i].name != NULL; i++) {
        ...
        if (strcmp(real_options[i].name, "data_dump_format") == 0) real_options[i].u.dst_ptr = &data_dump_format;
