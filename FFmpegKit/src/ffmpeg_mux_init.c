...
static int get_preset_file_2(const char *preset_name, const char *codec_name, AVIOContext **s)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(base) && ret < 0; i++) {
        ...
        if (codec_name) {
            ...
            ret = avio_open2(s, filename, AVIO_FLAG_READ, &interrupt_callback, NULL);
        }
        if (ret < 0) {
...
static FFMPEG_THREAD_LOCAL EncStatsFile   *enc_stats_files;
static FFMPEG_THREAD_LOCAL          int nb_enc_stats_files;
...
static int enc_stats_get_file(AVIOContext **io, const char *path)
{
    ...
    ret = avio_open2(&esf->io, path, AVIO_FLAG_WRITE, &interrupt_callback, NULL);
    if (ret < 0) {
...
static int of_add_attachments(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int i = 0; i < o->nb_attachments; i++) {
        ...
        if ((err = avio_open2(&pb, o->attachments[i], AVIO_FLAG_READ, &interrupt_callback, NULL)) < 0) {
...
int of_open(const OptionsContext *o, const char *filename, Scheduler *sch)
{
    ...
    ffmpeg_init_interrupt_callback(&oc->interrupt_callback);
