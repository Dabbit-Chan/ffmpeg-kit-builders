...
static std::set<std::string> split_csv(const std::string &value) {
    ...
    while (std::getline(stream, item, ',')) {
        ...
        const size_t length = (end == std::string::npos) ? std::string::npos : (end - start + 1);
        result.insert(item.substr(start, length));
    }
    return result;
}

TEST_F(PackagesTest, GetPackageName) {
    std::string packageName = take_string(ffmpeg_kit_packages_get_package_name());
    EXPECT_FALSE(packageName.empty());
    
    // Package name should be the same as bundle type
    std::string bundleType = take_string(ffmpeg_kit_packages_get_bundle_type());
...
TEST_F(PackagesTest, GetBuildConfiguration) {
    std::string config = take_string(ffmpeg_kit_packages_get_build_configuration());
...
TEST_F(PackagesTest, GetIsGpl) {
    ...
    bool isGpl1 = ffmpeg_kit_packages_get_is_gpl();
    bool isGpl2 = ffmpeg_kit_packages_get_is_gpl();
...
TEST_F(PackagesTest, GetIsNonFree) {
    // This should return either true or false consistently
    bool isNonFree1 = ffmpeg_kit_packages_get_is_nonfree();
    bool isNonFree2 = ffmpeg_kit_packages_get_is_nonfree();
    ...
    bool isGpl = ffmpeg_kit_packages_get_is_gpl();
    EXPECT_FALSE(isGpl && isNonFree1) << "GPL and NonFree should not both be enabled";
}

TEST_F(PackagesTest, GetExternalLibraries) {
    auto libraries = split_csv(take_string(ffmpeg_kit_packages_get_external_libraries()));
...
TEST_F(PackagesTest, GetRegisteredCodecs) {
    auto codecs = split_csv(take_string(ffmpeg_kit_packages_get_registered_codecs()));
...
TEST_F(PackagesTest, GetRegisteredEncoders) {
    auto encoders = split_csv(take_string(ffmpeg_kit_packages_get_registered_encoders()));
...
TEST_F(PackagesTest, GetRegisteredDecoders) {
    auto decoders = split_csv(take_string(ffmpeg_kit_packages_get_registered_decoders()));
...
TEST_F(PackagesTest, GetRegisteredMuxers) {
    auto muxers = split_csv(take_string(ffmpeg_kit_packages_get_registered_muxers()));
...
TEST_F(PackagesTest, GetRegisteredDemuxers) {
    auto demuxers = split_csv(take_string(ffmpeg_kit_packages_get_registered_demuxers()));
...
TEST_F(PackagesTest, GetRegisteredFilters) {
    auto filters = split_csv(take_string(ffmpeg_kit_packages_get_registered_filters()));
...
TEST_F(PackagesTest, GetRegisteredProtocols) {
    auto protocols = split_csv(take_string(ffmpeg_kit_packages_get_registered_protocols()));
...
TEST_F(PackagesTest, GetRegisteredBitStreamFilters) {
    auto bsfs = split_csv(take_string(ffmpeg_kit_packages_get_registered_bitstream_filters()));
...
TEST_F(PackagesTest, ConsistencyBetweenMethods) {
    ...
    auto libraries = split_csv(take_string(ffmpeg_kit_packages_get_external_libraries()));
    auto config = take_string(ffmpeg_kit_packages_get_build_configuration());
...
TEST_F(PackagesTest, MethodReturnValuesAreStable) {
    ...
    // Test basic methods
    std::string bundle1 = take_string(ffmpeg_kit_packages_get_bundle_type());
    std::string bundle2 = take_string(ffmpeg_kit_packages_get_bundle_type());
    EXPECT_EQ(bundle1, bundle2) << "Bundle type should be stable";
    
    std::string config1 = take_string(ffmpeg_kit_packages_get_build_configuration());
    std::string config2 = take_string(ffmpeg_kit_packages_get_build_configuration());
    EXPECT_EQ(config1, config2) << "Build configuration should be stable";
    
    bool gpl1 = ffmpeg_kit_packages_get_is_gpl();
    bool gpl2 = ffmpeg_kit_packages_get_is_gpl();
    EXPECT_EQ(gpl1, gpl2) << "GPL status should be stable";
    
    bool nonfree1 = ffmpeg_kit_packages_get_is_nonfree();
    bool nonfree2 = ffmpeg_kit_packages_get_is_nonfree();
    ...
    auto libs1 = split_csv(take_string(ffmpeg_kit_packages_get_external_libraries()));
    auto libs2 = split_csv(take_string(ffmpeg_kit_packages_get_external_libraries()));
    ...
    auto codecs1 = split_csv(take_string(ffmpeg_kit_packages_get_registered_codecs()));
    auto codecs2 = split_csv(take_string(ffmpeg_kit_packages_get_registered_codecs()));
