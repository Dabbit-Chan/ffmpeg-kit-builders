...
static void readrate_sleep(Demuxer *d)
{
    ...
    for (int i = 0; i < f->nb_streams; i++) {
        ...
        if ( (!ds->lag && lag > 0.3 * AV_TIME_BASE) || ( lag > ds->lag + 0.3 * AV_TIME_BASE) ) {
            ...
            ds->resume_wc = now;
            ds->resume_pts = pts;
            av_log_once(NULL, AV_LOG_WARNING, AV_LOG_DEBUG, &resume_warn,
                   ...
                   const char *pkt_desc)
{
    ...
    if (ret == AVERROR_EOF) {
        ...
        av_log(NULL, AV_LOG_VERBOSE, "All consumers of this stream are done\n");
        ds->finished = 1;

        if (++d->nb_streams_finished == d->nb_streams_used) {
            av_log(NULL, AV_LOG_VERBOSE, "All consumers are done\n");
            return AVERROR_EOF;
        }
    } else if (ret < 0) {
        if (ret != AVERROR_EXIT)
...
static int input_thread(void *arg)
{
    ...
    while (1) {
        ...
        if (ret < 0) {
            ...
            if (ret == AVERROR_EOF)
                av_log(NULL, AV_LOG_VERBOSE, "EOF while reading input\n");
            else {
                av_log(NULL, AV_LOG_ERROR, "Error during demuxing: %s\n",
                       av_err2str(ret));
    ...
    for (int j = 0; j < f->nb_streams; j++) {
        ...
        if (ds->decoding_needed) {
            ...
            if (type == AVMEDIA_TYPE_AUDIO)
                av_log(NULL, AV_LOG_VERBOSE, " (%"PRIu64" samples)", ist->decoder->samples_decoded);
            av_log(NULL, AV_LOG_VERBOSE, "; ");
        }
            ...
            const ViewSpecifier *vs, SchedulerNode *src)
{
    ...
    if (ist->user_set_discard == AVDISCARD_ALL) {
        av_log(NULL, AV_LOG_ERROR, "Cannot %s a disabled input stream\n",
                                      ...
                                      int guess_layout_max)
{
    if (par->ch_layout.order == AV_CHANNEL_ORDER_UNSPEC) {
        ...
        av_log(NULL, AV_LOG_WARNING, "Guessed Channel Layout: %s\n", layout_name);
    }
    return 1;
}
...
static int ist_add(const OptionsContext *o, Demuxer *d, AVStream *st, AVDictionary **opts_used)
{
    ...
    if (apply_cropping) {
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Invalid apply_cropping value '%s'.\n", apply_cropping);
            return ret;
        }
    }
    ...
    if (ds->drop_changed && ds->reinit_filters) {
        if (ds->reinit_filters > 0) {
            av_log(NULL, AV_LOG_ERROR, "drop_changed and reinit_filters both enabled. These are mutually exclusive.\n");
            return AVERROR(EINVAL);
        }
        ds->reinit_filters = 0;
    }
    ...
    case AVMEDIA_TYPE_AUDIO: {
        ...
        if (ch_layout_str) {
            ...
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR, "Error parsing channel layout %s.\n", ch_layout_str);
                return ret;
            }
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Error exporting stream parameters.\n");
        return ret;
    }
    ...
    if (bsfs) {
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error initializing bitstream filters: %s\n",
                   av_err2str(ret));
            return ret;
        }
...
static int dump_attachment(InputStream *ist, const char *filename)
{
    ...
    if (!st->codecpar->extradata_size) {
        av_log(NULL, AV_LOG_WARNING, "No extradata to dump.\n");
        return 0;
    }
    ...
    if (ret >= 0)
        av_log(NULL, AV_LOG_INFO, "Wrote attachment (%d bytes) to '%s'\n",
...
int ifile_open(const OptionsContext *o, const char *filename, Scheduler *sch)
{
    ...
    if (o->find_stream_info) {
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_FATAL, "could not find codec parameters\n");
            if (ic->nb_streams == 0)
                return ret;
        }
    }
    ...
    if (start_time_eof != AV_NOPTS_VALUE) {
        ...
        if (ic->duration > 0) {
            ...
            av_log(NULL, AV_LOG_WARNING, "Cannot use -sseof, file duration not known\n");
    }
    ...
    if (d->readrate < 0.0f) {
        av_log(NULL, AV_LOG_ERROR, "Option -readrate is %0.3f; it must be non-negative.\n", d->readrate);
        return AVERROR(EINVAL);
    }
    if (o->rate_emu) {
        if (d->readrate) {
            av_log(NULL, AV_LOG_WARNING, "Both -readrate and -re set. Using -readrate %0.3f.\n", d->readrate);
        } else
            d->readrate = 1.0f;
    }
    ...
    if (d->readrate) {
        ...
        if (o->readrate_initial_burst) {
            av_log(NULL, AV_LOG_WARNING, "Option -readrate_initial_burst ignored "
                   "since neither -readrate nor -re were given\n");
        }
        if (o->readrate_catchup) {
            av_log(NULL, AV_LOG_WARNING, "Option -readrate_catchup ignored "
                   "since neither -readrate nor -re were given\n");
        }
    }
    ...
    ret = check_avoptions_used(o->g->codec_opts, opts_used, NULL, 1);
    av_dict_free(&opts_used);
    if (ret < 0)
        return ret;
