                            ...
                            const ViewSpecifier *vs)
{
    ...
    if (ifilter->type != ist->par->codec_type &&
        ...
        av_log(NULL, AV_LOG_ERROR, "Tried to connect %s stream to %s filtergraph input\n",
                            ...
                            const ViewSpecifier *vs)
{
    ...
    if (ifp->ifilter.type != dec->type) {
        av_log(NULL, AV_LOG_ERROR, "Tried to connect %s decoder to %s filtergraph input\n",
...
static int ifilter_bind_fg(InputFilterPriv *ifp, FilterGraph *fg_src, int out_idx)
{
    ...
    if (ifp->ifilter.type != ofilter_src->type) {
        av_log(NULL, AV_LOG_ERROR, "Tried to connect %s output to %s input\n",
...
int fg_create(FilterGraph **pfg, char *graph_desc, Scheduler *sch)
{
    ...
    if (!fg->nb_outputs) {
        av_log(NULL, AV_LOG_FATAL, "A filtergraph has zero outputs, this is not supported\n");
        ret = AVERROR(ENOSYS);
        goto fail;
    }
                     ...
                     const OutputFilterOptions *opts)
{
    ...
    if (fg->outputs[0]->type != type) {
        av_log(NULL, AV_LOG_ERROR, "Filtergraph has a %s output, cannot connect "
...
static int fg_complex_bind_input(FilterGraph *fg, InputFilter *ifilter)
{
    ...
    if (ifilter->linklabel && !strncmp(ifilter->linklabel, "dec:", 4)) {
        ...
        if (dec_idx < 0 || dec_idx >= nb_decoders) {
            av_log(NULL, AV_LOG_ERROR, "Invalid decoder index %d in filtergraph description %s\n",
        ...
        if (ret < 0)
            av_log(NULL, AV_LOG_ERROR, "Error binding a decoder to filtergraph input %s\n",
                   ifilter->name);
        return ret;
        ...
        for (int i = 0; i < nb_filtergraphs; i++) {
            ...
            for (int j = 0; j < fg_src->nb_outputs; j++) {
                ...
                if (!ofilter->bound && ofilter->linklabel &&
                    ...
                    if (ret < 0)
                        av_log(NULL, AV_LOG_ERROR, "Error binding filtergraph input %s\n",
                               ifilter->linklabel);
                    return ret;
                }
            }
                       ...
                       const char *filter_name)
{
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Error configuring the %s filter", name);
        return ret;
    }
                               ...
                               int64_t *nb_frames, int64_t *nb_frames_prev)
{
    ...
    sync_ipts = adjust_frame_pts_to_encoder_tb(NULL, frame,
                          ...
                          AVFrame *frame)
{
    ...
    if (!ofp->tb_out_locked) {
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Could not choose an output time base\n");
            av_frame_unref(frame);
            return ret;
        }
    }
                       ...
                       AVFrame *frame)
{
    ...
    while (fgp->nb_outputs_done < fg->nb_outputs) {
        ...
        if (ret == AVERROR(EAGAIN)) {
            ...
            if (ret == AVERROR_EOF)
                av_log(NULL, AV_LOG_VERBOSE, "Filtergraph returned EOF, finishing\n");
            else
                av_log(NULL, AV_LOG_ERROR,
                      ...
                      InputFilter *ifilter, AVFrame *frame)
{
    ...
    if (need_reinit && fgt->graph && (ifp->opts.flags & IFILTER_FLAG_DROPCHANGED)) {
            ifp->nb_dropped++;
            av_log_once(NULL, AV_LOG_WARNING, AV_LOG_DEBUG, &ifp->drop_warned, "Avoiding reinit; dropping frame pts: %s bound for %s\n", av_ts2str(frame->pts), ifilter->name);
            av_frame_unref(frame);
            return 0;
    }
    ...
    if (need_reinit || !fgt->graph) {
        ...
        if (fgt->graph) {
            ...
            av_log(NULL, AV_LOG_INFO, "Reconfiguring filter graph%s%s\n", reason.len ? " because " : "", reason.str);
        }
...
static int filter_thread(void *arg)
{
    ...
    while (1) {
        ...
        if (input_status == AVERROR_EOF) {
            av_log(NULL, AV_LOG_VERBOSE, "Filtering thread received EOF\n");
            break;
        ...
        av_frame_unref(fgt.frame);
        if (ret == AVERROR_EOF) {
            av_log(NULL, AV_LOG_VERBOSE, "Input %u no longer accepts new data\n",
                   input_idx);
...
read_frames:
        ...
        if (ret == AVERROR_EOF) {
            av_log(NULL, AV_LOG_VERBOSE, "All consumers returned EOF\n");
            if (ifp && ifp->opts.flags & IFILTER_FLAG_DROPCHANGED)
                av_log(NULL, AV_LOG_INFO, "Total changed input frames dropped : %"PRId64"\n", ifp->nb_dropped);
            break;
        } else if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error sending frames to consumers: %s\n",
                   av_err2str(ret));
            goto finish;
        }
    }
