    ...
    *enc = NULL;
    ...
    if (!codec_name) {
        ...
        *enc = avcodec_find_encoder(ms->par_in->codec_id);
        if (!*enc) {
            av_log(NULL, AV_LOG_FATAL, "Automatic encoder selection failed "
                           ...
                           OutputStream *ost, char **dst)
{
    ...
    if (filters_script && filters) {
        av_log(NULL, AV_LOG_ERROR, "Both -filter and -filter_script set\n");
        return AVERROR(EINVAL);
    }
    ...
    if (fmts && !fmt_in_list(fmts, fmt)) {
        ...
        if (strcmp(name, name_canonical) &&
            ...
            if (fmt_other != AV_PIX_FMT_NONE && fmt_in_list(fmts, fmt_other)) {
                av_log(NULL, AV_LOG_VERBOSE, "Mapping pixel format %s->%s\n",
                       name, name_other);
                fmt = fmt_other;
            }
        }
                            ...
                            enum VideoSyncMethod *vsync_method)
{
    ...
    if (ost->enc) {
        ...
        if (intra_matrix) {
            ...
            ret = parse_matrix_coeffs(NULL, video_enc->intra_matrix, intra_matrix);
            if (ret < 0)
                return ret;
        }
        ...
        if (chroma_intra_matrix) {
            if (!(video_enc->chroma_intra_matrix = av_mallocz(sizeof(*video_enc->chroma_intra_matrix) * 64)))
                return AVERROR(ENOMEM);
            ret = parse_matrix_coeffs(NULL, video_enc->chroma_intra_matrix, chroma_intra_matrix);
            if (ret < 0)
                return ret;
        }
        ...
        if (inter_matrix) {
            if (!(video_enc->inter_matrix = av_mallocz(sizeof(*video_enc->inter_matrix) * 64)))
                return AVERROR(ENOMEM);
            ret = parse_matrix_coeffs(NULL, video_enc->inter_matrix, inter_matrix);
            if (ret < 0)
                return ret;
        }
        ...
        if (ost->top_field_first >= 0)
            av_log(NULL, AV_LOG_WARNING, "-top is deprecated, use the setfield filter instead\n");
#endif
                   ...
                   OutputStream **post)
{
    ...
    if (o->streamid) {
        ...
        if (e) {
            ...
            if (!e->value[0] || *p) {
                av_log(NULL, AV_LOG_FATAL, "Invalid stream id: %s\n", e->value);
                return AVERROR(EINVAL);
            }
        }
    }
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_FATAL, "Error selecting an encoder\n");
        return ret;
    }
    ...
    av_log(NULL, AV_LOG_VERBOSE, "Created %s stream from ",
           av_get_media_type_string(type));
    if (ist)
        av_log(NULL, AV_LOG_VERBOSE, "input stream %d:%d",
               ist->file->index, ist->index);
    else if (ofilter)
        av_log(NULL, AV_LOG_VERBOSE, "complex filtergraph %d:[%s]\n",
               ofilter->graph->index, ofilter->name);
    else if (type == AVMEDIA_TYPE_ATTACHMENT)
        av_log(NULL, AV_LOG_VERBOSE, "attached file");
    else av_assert0(0);
    ...
    if (ost->enc) {
...
#if FFMPEG_OPT_ENC_TIME_BASE_NUM
                if (q.num < 0)
                    av_log(NULL, AV_LOG_WARNING, "-enc_time_base -1 is deprecated,"
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error applying encoder options: %s\n",
                   av_err2str(ret));
            goto fail;
        }
...
loop_end:
        ...
        av_log(NULL, AV_LOG_VERBOSE, "Creating output stream from an explicitly "
...
static int of_add_attachments(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int i = 0; i < o->nb_attachments; i++) {
        ...
        if (err < 0)
            av_log(NULL, AV_LOG_FATAL, "Error reading attachment file %s: %s\n",
                   o->attachments[i], av_err2str(err));
        else if (err != len) {
            av_log(NULL, AV_LOG_FATAL, "Could not read all %"PRId64" bytes for "
                   "attachment file %s\n", len, o->attachments[i]);
            err = AVERROR(EIO);
        }
...
read_fail:
        ...
        av_log(NULL, AV_LOG_VERBOSE, "Creating attachment stream from file %s\n",
               o->attachments[i]);
...
static int create_streams(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int i = 0; i < nb_filtergraphs; i++) {
        ...
        for (int j = 0; j < fg->nb_outputs; j++) {
            ...
            av_log(NULL, AV_LOG_VERBOSE, "Creating output stream from unlabeled "
                   "output of complex filtergraph %d.", fg->index);
            if (!o->nb_stream_maps)
                av_log(NULL, AV_LOG_VERBOSE, " This overrides automatic %s mapping.",
    ...
    if (!o->nb_stream_maps) {
        av_log(NULL, AV_LOG_VERBOSE, "No explicit maps, mapping streams automatically...\n");
        ...
        av_log(NULL, AV_LOG_VERBOSE, "Adding streams from explicit maps...\n");
    ...
    if (!oc->nb_streams && !(oc->oformat->flags & AVFMT_NOSTREAMS)) {
        ...
        av_log(NULL, AV_LOG_ERROR, "Output file does not contain any stream\n");
        return AVERROR(EINVAL);
    }
...
static int of_parse_iamf_audio_element_layers(Muxer *mux, AVStreamGroup *stg, char *ptr)
{
    ...
    while (token) {
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error parsing audio element specification %s\n", token);
            goto fail;
        }
        ...
        if (layer) {
            ...
            if (!audio_layer) {
                av_log(NULL, AV_LOG_ERROR, "Error adding layer to stream group %d\n", stg->index);
                ret = AVERROR(ENOMEM);
                goto fail;
            }
        ...
        if (e = av_dict_iterate(dict, e)) {
            av_log(NULL, AV_LOG_FATAL, "Unknown layer key %s.\n", e->key);
            ret = AVERROR(EINVAL);
            goto fail;
        }
        token = av_strtok(NULL, ",", &ptr);
...
fail:
    ...
    if (!ret && !audio_element->nb_layers) {
        av_log(NULL, AV_LOG_ERROR, "No layer in audio element specification\n");
        ret = AVERROR(EINVAL);
    }
...
static int of_parse_iamf_submixes(Muxer *mux, AVStreamGroup *stg, char *ptr)
{
    ...
    while (token) {
        ...
        if (!av_strstart(token, "submix=", &token)) {
            av_log(NULL, AV_LOG_ERROR, "No submix in mix presentation specification \"%s\"\n", token);
            goto fail;
        }
        ...
        if (!submix) {
            av_log(NULL, AV_LOG_ERROR, "Error adding submix to stream group %d\n", stg->index);
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        while (subtoken) {
            ...
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR, "Error parsing submix specification \"%s\"\n", subtoken);
                goto fail;
            }
            ...
            if (element) {
                ...
                if (!endptr || *endptr || idx < 0 || idx >= oc->nb_stream_groups - 1 ||
                    ...
                    av_log(NULL, AV_LOG_ERROR, "Invalid or missing stream group index in "
                ...
                if (!submix_element) {
                    av_log(NULL, AV_LOG_ERROR, "Error adding element to submix\n");
                    ret = AVERROR(ENOMEM);
                    goto fail;
                }
                ...
                if (!submix_layout) {
                    av_log(NULL, AV_LOG_ERROR, "Error adding layout to submix\n");
                    ret = AVERROR(ENOMEM);
                    goto fail;
                }
            ...
            while (e = av_dict_iterate(dict, e)) {
                av_log(NULL, AV_LOG_FATAL, "Unknown submix key %s.\n", e->key);
                ret = AVERROR(EINVAL);
                goto fail;
            }
        ...
        if (!submix->nb_elements) {
            av_log(NULL, AV_LOG_ERROR, "No audio elements in submix specification \"%s\"\n", token);
            ret = AVERROR(EINVAL);
        }
        token = av_strtok(NULL, ",", &ptr);
    }
...
static int of_serialize_options(Muxer *mux, void *obj, AVBPrint *bp)
{
    ...
    ret = av_opt_serialize(obj, 0, AV_OPT_SERIALIZE_SKIP_DEFAULTS | AV_OPT_SERIALIZE_SEARCH_CHILDREN,
                           &ptr, '=', ':');
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Failed to serialize group\n");
        return ret;
    }
...
static int of_map_group(Muxer *mux, AVDictionary **dict, AVBPrint *bp, const char *map)
{
    ...
    if (*ptr || stream_idx >= input_files[file_idx]->ctx->nb_stream_groups || stream_idx < 0) {
        av_log(NULL, AV_LOG_ERROR, "Invalid input stream group index: %d.\n", stream_idx);
        return AVERROR(EINVAL);
    }
    ...
    if (ret < 0)
        av_log(NULL, AV_LOG_ERROR, "Error parsing mapped group specification %s\n", ptr);
    ...
    default:
        av_log(NULL, AV_LOG_ERROR, "Unsupported mapped group type %d.\n", stg->type);
        ret = AVERROR(EINVAL);
        break;
    }
    return 0;
...
static int of_parse_group_token(Muxer *mux, const char *token, char *ptr)
{
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Error parsing group specification %s\n", token);
        return ret;
    }
    ...
    if (e) {
        ...
        if (ptr) {
            av_log(NULL, AV_LOG_ERROR, "Unexpected extra parameters when mapping a"
                                      " stream group\n");
            ret = AVERROR(EINVAL);
            goto end;
        }
    ...
    if (!e) {
        av_log(NULL, AV_LOG_ERROR, "No type specified for Stream Group in \"%s\"\n", token);
        ret = AVERROR(EINVAL);
        goto end;
    }
    ...
    if (!ret && type == AV_STREAM_GROUP_PARAMS_NONE)
        ret = AVERROR(EINVAL);
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Invalid group type \"%s\"\n", e->value);
        goto end;
    }
    ...
    while (e = av_dict_get(dict, "st", e, 0)) {
        ...
        if (*endptr || idx < 0 || idx >= oc->nb_streams) {
            av_log(NULL, AV_LOG_ERROR, "Invalid stream index %"PRId64"\n", idx);
            ret = AVERROR(EINVAL);
            goto end;
        }
    ...
    while (e = av_dict_get(dict, "stg", e, 0)) {
        ...
        if (*endptr || idx < 0 || idx >= oc->nb_stream_groups - 1) {
            av_log(NULL, AV_LOG_ERROR, "Invalid stream group index %"PRId64"\n", idx);
            ret = AVERROR(EINVAL);
            goto end;
        }
    ...
    case AV_STREAM_GROUP_PARAMS_IAMF_MIX_PRESENTATION:
        ...
        break;
    default:
        av_log(NULL, AV_LOG_FATAL, "Unknown group type %d.\n", type);
        ret = AVERROR(EINVAL);
        break;
    }
    ...
    while (e = av_dict_iterate(tmp, e)) {
        ...
        av_log(NULL, AV_LOG_FATAL, "Unknown group key %s.\n", e->key);
        ret = AVERROR(EINVAL);
        goto end;
    }
...
static int of_add_programs(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int i = 0; i < o->program.nb_opt; i++) {
        ...
        ret = av_dict_parse_string(&dict, o->program.opt[i].u.str, "=", ":",
                                   AV_DICT_MULTIKEY);
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error parsing program specification %s\n",
                           ...
                           const OptionsContext *o)
{
    for (int i = 0; i < o->metadata.nb_opt; i++) {
        ...
        *val++ = 0;
        ...
        ret = parse_meta_type(NULL, o->metadata.opt[i].specifier, &type, &index, &stream_spec);
        if (ret < 0)
            return ret;
                         ...
                         int *metadata_chapters_manual)
{
    ...
    ret     = parse_meta_type(NULL, inspec,  &type_in,  &idx_in,  &istream_spec);
    if (ret >= 0)
        ret = parse_meta_type(NULL, outspec, &type_out, &idx_out, &ostream_spec);
    if (ret < 0)
        return ret;
...
static int copy_meta(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int i = 0; i < o->metadata_map.nb_opt; i++) {
        ...
        if (in_file_index >= nb_input_files) {
            av_log(NULL, AV_LOG_FATAL, "Invalid input file index %d while "
...
static int process_forced_keyframes(Muxer *mux, const OptionsContext *o)
{
    for (int i = 0; i < mux->of.nb_streams; i++) {
        ...
        if (!strncmp(forced_keyframes, "expr:", 5)) {
            ...
            av_log(NULL, AV_LOG_WARNING, "The 'source_no_drop' value for "
...
static void validate_enc_avopt(Muxer *mux, const AVDictionary *codec_avopt)
{
    ...
    while ((e = av_dict_iterate(unused_opts, e))) {
        ...
        if (!(option->flags & AV_OPT_FLAG_ENCODING_PARAM)) {
            av_log(NULL, AV_LOG_ERROR, "Codec AVOption %s (%s) is not an "
        ...
        av_log(NULL, AV_LOG_WARNING, "Codec AVOption %s (%s) has not been used "
...
static int init_output_stream_nofilter(OutputStream *ost)
{
    ...
    if (ost->enc->enc_ctx) {
        ...
        ret = of_stream_init(output_files[ost->file->index], ost, NULL);
        if (ret < 0)
            return ret;
    }
...
int of_open(const OptionsContext *o, const char *filename, Scheduler *sch)
{
    ...
    if (!oc) {
        av_log(NULL, AV_LOG_FATAL, "Error initializing the muxer for %s: %s\n",
    ...
    if (!(oc->oformat->flags & AVFMT_NOFILE)) {
        ...
        if ((err = avio_open2(&oc->pb, filename, AVIO_FLAG_WRITE,
                              ...
                              &mux->opts)) < 0) {
            av_log(NULL, AV_LOG_FATAL, "Error opening output %s: %s\n",
                   filename, av_err2str(err));
            return err;
        }
